...
def minimize_label(label: str) -> int:
    if label == "Cloudy_Low":
        return 1
    ...
    try:
        ...
        if data:
            ...
            # -------- CAPTURA --------
                ...
                nombre = "images/capture_%d%d%d_%d%d%d.jpg" % (
                    fecha_hora[0],
                    fecha_hora[1],
                    fecha_hora[2],
                    fecha_hora[4],
                ...
                for i in range(total_pkts):
                    ...
                    print(
                        "Esperando reloj del Maestro para paquete", i + 1
                    )  # Enviar una respuesta con un caracter para activar la comunicación SPI
                    try:
                        # Mandamos el fragmento por SPI
                        # Modular el sistema para que no dependa de un solo comando
                        spi.send(
                            fragmento, timeout=2000
                        )  # Deben coincidir los timeout para evitar errores de envio
                    except Exception as e:
            ...
            # -------- FUNCION DE AUTOMATIZACIÓN DEL SISTEMA ---------
                ...
                while num_imagenes == num_img_act:
                    ...
                    if rand_arch < 3:  # Envio aleatorio de 3/4 archivos de imagen
                        ...
                        rand_arch == 3
                    ):  # Se eliminan los archivos de la matriz (send_imgs)
                        rand_arch = 0
            ...
            # ------ FUNCIONES EXTRAS -------
                ...
                response = "C:{} B:{} M:{}".format(contrast_val, brightness_val, mode)
            elif cmd == "i":
                response = "{}{:.2f}{}{}".format(
                    minimize_label(best_label), best_score, detected_flag, saved_flag
                )
            elif cmd == "p":
                response = "PING"
